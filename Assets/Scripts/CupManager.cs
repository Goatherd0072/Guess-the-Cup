...
public class CupManager : MonoBehaviour
{
    ...
    {
        if (!PlayerManager.m_PlayerManager.IsOver)
        {
            if (Input.GetMouseButtonDown(0))
            {
                ...
                if (Physics.Raycast(r, out RaycastHit hitInfor, 100))
                {
                    ...
                    CupUnit hitObject = hitInfor.collider.GetComponent<CupUnit>();
                    if (hitObject)
                    {
    ...
    void CreateObject()
    {
        ...
        for (int j = 0; j < line; j++)//生产行
        {
            ...
            for (int i = 0; i < 5; i++)//生成列
            {
                ...
                CupUnit unit = Instantiate<CupUnit>(object_Clone, pos, Quaternion.identity);
                unit.name = j + "-" + i;
                All_Cup.Add(unit);
                unit.Init(false);
                tempNum++;
